...
def get_intent(project_id, intent_name):
    intents_client = dialogflow.IntentsClient()
    parent = dialogflow.AgentsClient.agent_path(project_id)
    intent = intents_client.get_intent(request={"name": "projects/"+ config["project_id"] + "/agent/intents/" + intent_name})
    return intent
...
def create_intent(project_id, display_name, training_phrases_parts, message_texts, parameters=[]):
    ...
    if len(parameters) > 0:
        ...
        for parameter in parameters:
            ...
            DFParameter = dialogflow.Intent.Parameter(
                name="",
            ...
            newParameters.append(DFParameter)
...
for intent in list_intents(config['project_id']):
    print("{}".format(intent.display_name))
    # print("\n")
...
for filename in filenames:
    ...
    if filename.endswith(".json"):
        ...
        # print("{}\n\n{}\n\n{}\n\n{}".format(intentName, trainingPhrases, responses, parametersyay))
