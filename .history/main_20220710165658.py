    ...
    for filename in filenames:
        ...
        if filename.endswith(".json"):
            ...
            try:
                ...
                exit()
            os.rename("./intents/" + filename, "./intents/done/" + re.sub(".json","",filename) + "-added-" + re.sub(":",".",re.sub(" ", "_", time.strftime("%x_%X"))) + ".json")
    ...
    for filename in filenames:
        ...
        if filename.endswith(".json"):
            ...
            if len(data["userSays"]) == 0:
                print("{} is invalid, Reason: {}".format(filename, "intent doesn't contain any userSays"))
                exit()
        print("Deleting intent from {}...".format(filename))
        intentName = data["name"]
        if get_intent(config['project_id'], intentName) is None:
            print("{} doesn't exist, skipping...".format(intentName))
            continue
        try:
            ...
            print("{} failed to delete intent, Reason: {}".format(intentName, e))
            exit()
        os.rename("./intents/" + filename, "./intents/done/" + re.sub(".json","",filename) + "-deleted-" + re.sub(":",".",re.sub(" ", "_", time.strftime("%x_%X"))) + ".json")
def banner():
    print("""
==========================================================
